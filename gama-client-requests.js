...
function addMenuCard(grid,id,icon,title,action){
  ...
  card.type='button';card.id=id;card.dataset.gamaShortcut='1';
  ...
  card.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();try{action(e)}catch(err){console.error('[GAMA] shortcut error',err)}},{passive:false});
...
function scheduleInstall(){
  [0,40,120,300,700,1200].forEach(ms=>setTimeout(install,ms));
}
function observeMenu(){
  const host=document.getElementById('mainmenu');
  if(host&&!menuObserver){
    menuObserver=new MutationObserver(()=>scheduleInstall());
...
function hookShowTab(){
  if(typeof window.showTab!=='function'||window.showTab.__gamaClientShortcutHook)return;
  const original=window.showTab;
  const wrapped=function(){
    const result=original.apply(this,arguments);
    if(arguments[0]==='mainmenu')scheduleInstall();
    return result;
  };
  wrapped.__gamaClientShortcutHook=true;
  window.showTab=wrapped;
}
function boot(){
  if(loading)return;loading=true;
  let pending=2,done=()=>{pending--;if(pending<=0){loading=false;install();observeMenu();hookShowTab();scheduleInstall()}};
  ...
  if(!bodyObserver){
    bodyObserver=new MutationObserver(()=>{observeMenu();hookShowTab()});
