(function(){
  ...
  function addStyle(){
    ...
    s.textContent='.gamaDevNativeTile{position:relative!important;min-height:138px!important;background:#fff!important;border:1px solid #E2E8EC!important;border-radius:14px!important;padding:14px 10px 12px!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;gap:7px!important;color:#18324A!important;box-shadow:0 3px 13px rgba(24,50,74,.07)!important;cursor:pointer!important}.gamaDevNativeIcon{width:58px;height:58px;border-radius:15px;display:grid;place-items:center;font-size:31px}.gamaDevNativeTile.orange .gamaDevNativeIcon{background:#FFF0E7}.gamaDevNativeTile.teal .gamaDevNativeIcon{background:#E8F5F6}.gamaDevNativeBadge{position:absolute;right:10px;top:10px;padding:5px 9px;border-radius:999px;color:#fff;font-size:9px;font-weight:900}.gamaDevNativeTile.orange .gamaDevNativeBadge{background:#F47A2A}.gamaDevNativeTile.teal .gamaDevNativeBadge{background:#2E8790}.gamaDevNativeTitle{font-size:14px;font-weight:900;text-align:center;line-height:1.15}.gamaDevNativeDesc{font-size:10px;color:#71808A;text-align:center;line-height:1.2}@media(max-width:900px){.gamaDevNativeTile{min-height:132px}}@media(max-width:700px){.gamaDevNativeTile{min-height:118px;border-radius:12px;padding:10px 5px}.gamaDevNativeIcon{width:47px;height:47px;border-radius:13px;font-size:27px}.gamaDevNativeTitle{font-size:12px}.gamaDevNativeDesc{font-size:9px}}';
    document.head.appendChild(s);
  }
  function moduleNotice(title){
    let box=document.getElementById('gamaDevModuleNotice');
    if(!box){
      box=document.createElement('div');box.id='gamaDevModuleNotice';
      box.style.cssText='position:fixed;inset:0;z-index:100000;background:rgba(15,35,50,.45);display:flex;align-items:center;justify-content:center;padding:20px';
      box.innerHTML='<div style="max-width:460px;width:100%;background:#fff;border-radius:20px;padding:24px;box-shadow:0 15px 50px #0004"><div id="gamaDevNoticeTitle" style="font-size:20px;font-weight:900;color:#18324A;margin-bottom:8px"></div><div style="color:#71808A;font-size:13px">Module DEV sélectionné. Le raccordement fonctionnel du catalogue et des demandes sera effectué dans cette interface.</div><button type="button" style="margin-top:18px;background:#087C8B;color:#fff;border:0;border-radius:10px;padding:10px 16px;font-weight:800" onclick="document.getElementById(\'gamaDevModuleNotice\').remove()">Fermer</button></div>';
  ...
  function install(){
    const grid=document.querySelector('#mainmenu .appGrid')||document.querySelector('.appGrid');
    ...
    const ok=TILES.every(function(t){return !!document.getElementById(t.id)});status.textContent=ok?'✓ 2/2 tuiles DEV affichées':'⚠ Tuiles DEV en attente';return ok;
  }
