...
(function(){
  ...
  async function load(){
    ...
    try{status.textContent='Synchronisation…';const r=await window.GamaCloud.list('profiles',{order:'created_at',ascending:false});if(r.error)throw r.error;const rows=r.data||[];body.innerHTML=rows.length?rows.map(x=>`<tr><td><b>${esc(x.full_name||'Sans nom')}</b></td><td>${esc(x.email||'—')}</td><td><span class="cuBadge">${esc(ROLE[x.role]||x.role||'Utilisateur')}</span></td><td class="${x.active===false?'cuInactive':'cuActive'}">${x.active===false?'● Désactivé':'● Actif'}</td><td>${x.created_at?new Date(x.created_at).toLocaleString('fr-FR'):'—'}</td><td class="cuId">${esc(x.id)}</td></tr>`).join(''):'<tr><td colspan="6">Aucun utilisateur.</td></tr>';s.querySelector('#cuCount').textContent=rows.length+' utilisateur'+(rows.length>1?'s':'');status.textContent='Dernière synchronisation : '+new Date().toLocaleTimeString('fr-FR')}catch(e){console.error('[GAMA Cloud Users]',e);status.textContent='Erreur cloud : '+(e.message||e);body.innerHTML='<tr><td colspan="6" class="cuInactive">Impossible de lire profiles. Vérifiez les policies RLS.</td></tr>'}
  }
  function init(){
