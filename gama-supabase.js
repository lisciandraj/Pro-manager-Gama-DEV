...
(function(){
  ...
  function loadSDK(){
    ...
    if(window.__gamaSupabaseSDK)return window.__gamaSupabaseSDK;
    window.__gamaSupabaseSDK=new Promise((resolve,reject)=>{
      const s=document.createElement('script');
      s.src='https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';
      s.async=true;
      s.onload=()=>window.supabase?.createClient?resolve(window.supabase):reject(Error('Supabase JS unavailable'));
      s.onerror=()=>reject(Error('Unable to load Supabase JS'));
      document.head.appendChild(s);
    });
    return window.__gamaSupabaseSDK;
  }
  async function init(){
    if(client)return client;
    const sb=await loadSDK();
    client=sb.createClient(SUPABASE_URL,SUPABASE_PUBLISHABLE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});
    client.auth.onAuthStateChange((event,session)=>emit('gama:auth-change',{event,session}));
  ...
  async function signOut(){
    const c=await db();
    try{return await c.auth.signOut({scope:'local'})}
    finally{try{localStorage.removeItem('gama_session_v1');sessionStorage.removeItem('gama_session_v1')}catch(_){}
      channels.forEach(ch=>{try{c.removeChannel(ch)}catch(_){} });channels=[];}
  }
  async function getProfile(){
    const c=await db(),u=(await c.auth.getUser()).data?.user;
    if(!u)return {data:null,error:null};
    return c.from('profiles').select('*').eq('id',u.id).maybeSingle();
  }
  async function list(table,options={}){
    let q=(await db()).from(table).select(options.select||'*');
  ...
  async function subscribe(table,callback){
    const c=await db();
    const channel=c.channel('gama-'+table+'-'+Date.now()).on('postgres_changes',{event:'*',schema:'public',table},payload=>{emit('gama:data-change',{table,payload});callback?.(payload)}).subscribe();
  ...
  window.GamaCloudReady=init().then(async()=>{
    const modules=['gama-cloud-products.js?v=19','gama-cloud-users.js?v=18','gama-purchases-supplier-bridge.js?v=6','gama-invoice-archive.js?v=3','gama-client-catalog.js?v=6','gama-customer-requests.js?v=3'];
    for(const src of modules){
      if(document.querySelector('script[src^="'+src.split('?')[0]+'"]'))continue;
      await new Promise(resolve=>{const s=document.createElement('script');s.src=src;s.async=false;s.onload=resolve;s.onerror=()=>{console.warn('[GAMA] Module unavailable:',src);resolve()};document.head.appendChild(s)});
    }
